...
def acceleration(X,L,V0,rho,mu):
	...
	DUDt[:,0] = (-V0/L) * ( (V0/8)*np.sin(2*X[:,0]/L)*np.cos(2*X[:,2]/L) + (2*mu/(rho*L))*np.sin(X[:,0]/L)*np.cos(X[:,1]/L)*np.cos(X[:,2]/L) )
	DUDt[:,1] = (-V0/L) * ( (V0/8)*np.sin(2*X[:,1]/L)*np.cos(2*X[:,2]/L) + (2*mu/(rho*L))*np.cos(X[:,0]/L)*np.sin(X[:,1]/L)*np.cos(X[:,2]/L) )
